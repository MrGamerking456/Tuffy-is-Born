...
public class ErrorReporter implements UncaughtExceptionHandler {
    ...
    class C03183 extends Thread {
        ...
        C03183(SendWorker sendWorker, boolean z, String str, boolean z2) {
        ...
        L_0x0019:
            r2 = 100;
            java.lang.Thread.sleep(r2);	 Catch:{ InterruptedException -> 0x001f }
            goto L_0x0007;
        L_0x001f:
            r0 = move-exception;
            r1 = org.acra.ACRA.LOG_TAG;
    ...
    private void handleException(Throwable e, ReportingInteractionMode reportingInteractionMode, boolean forceSilentReport, boolean endApplication) {
        ...
        if (this.enabled) {
            ...
            new C03183(worker, showDirectDialog, reportFileName, endApplication).start();
        }
    }
    ...
    private String getReportFileName(CrashReportData crashData) {
        Time now = new Time();
        now.setToNow();
        return ElementType.MATCH_ANY_LOCALNAME + now.toMillis(false) + (crashData.getProperty(ReportField.IS_SILENT) != null ? ACRAConstants.SILENT_SUFFIX : ElementType.MATCH_ANY_LOCALNAME) + ACRAConstants.REPORTFILE_EXTENSION;
    }
    ...
    public void setDefaultReportSenders() {
        ...
        removeAllReportSenders();
        if (!ElementType.MATCH_ANY_LOCALNAME.equals(conf.mailTo())) {
        ...
        } else if (conf.formUri() != null && !ElementType.MATCH_ANY_LOCALNAME.equals(conf.formUri())) {
            setReportSender(new HttpPostSender(null));
        } else if (conf.formKey() != null && !ElementType.MATCH_ANY_LOCALNAME.equals(conf.formKey().trim())) {
