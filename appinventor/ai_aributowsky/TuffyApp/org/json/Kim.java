...
public class Kim {
        ...
        if (stringLength > 0) {
            ...
            while (i < stringLength) {
                ...
                if (character <= 127) {
                    ...
                    b = (character >>> 7) | DateTime.TIMEZONE_MASK;
                    ...
                    if (character >= 55296 && character <= 56319) {
                        i++;
                        character = (((character & 1023) << 10) | (string.charAt(i) & 1023)) + ModuleExp.NONSTATIC_SPECIFIED;
                    }
                    b = (character >>> 14) | DateTime.TIMEZONE_MASK;
                    this.bytes[at] = (byte) b;
                    sum += b;
                    this.hashcode += sum;
                    at++;
                    b = ((character >>> 7) & 255) | DateTime.TIMEZONE_MASK;
                    this.bytes[at] = (byte) b;
                    sum += b;
                    this.hashcode += sum;
                    at++;
    ...
    public int characterAt(int at) throws JSONException {
        int c = get(at);
        if ((c & DateTime.TIMEZONE_MASK) == 0) {
            return c;
        }
        int c1 = get(at + 1);
        int character;
        if ((c1 & DateTime.TIMEZONE_MASK) == 0) {
        ...
        if ((c2 & DateTime.TIMEZONE_MASK) == 0 && character > 16383 && character <= 1114111) {
            if (character < 55296) {
                return character;
            }
            if (character > 57343) {
    ...
    public String toString() throws JSONException {
        if (this.string == null) {
            ...
            while (at < this.length) {
                ...
                if (c < ModuleExp.NONSTATIC_SPECIFIED) {
                    chars[length] = (char) c;
                    length++;
                } else {
                    chars[length] = (char) (55296 | ((c - ModuleExp.NONSTATIC_SPECIFIED) >>> 10));
                    length++;
